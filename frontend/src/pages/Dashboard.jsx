...
import { HomeIcon,CalendarIcon,Filter,Plus } from 'lucide-react'
...
const Dashboard = () => {
    ...
    const stats =useMemo(() =>({
        ...
        // normalize to string
        const status = typeof t.completed === 'string' ? t.completed.toLowerCase() : t.completed;
...
const handleTaskSave = useCallback(async (taskData) => {
    try {
        if (taskData.id) await axios.put(`${API_URL}/${taskData.id}/gp`, taskData)
        refreshTasks()
        setShowModel(false)
        setSelectTask(null)
    } catch (error) {
                    ...
                    {filteredTasks.length===0 ? (
                        <div className={EMPTY_STATE.wrapper}>
                            <div className={EMPTY_STATE.iconWrapper}>
                                ...
                                <p className='text-sm text-gray-500 mb-4'>{filter==="all" ? "Create your first task to get started" : "No Tasks"}
